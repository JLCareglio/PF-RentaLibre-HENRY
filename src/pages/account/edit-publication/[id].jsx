...
export default function EditPublication() {
    ...
    function handleSelectFile(file) {
        setPublicationPictures({ ...publicationPictures,[editPublication.picturePosition]:[true,'']});
        ...
        uploadTask.on(
          ...
          () => {
            getDownloadURL(uploadTask.snapshot.ref).then((url) => {
             setEditPublication({ ...editPublication,saveButton:true,picture:url});
            setPublicationPictures({ ...publicationPictures,[editPublication.picturePosition]:[false,url]});
            });
          }
        );
      }
    ...
    function handleSaveClick() {
      const categoryId = categoriesDB.find((c)=>c.name === editPublication.category)
      let picturesUrl = []
      ...
      updateProduct.mutateAsync({
        pictures: picturesUrl[0] ? picturesUrl : product?.pictures ? product.pictures : [''],
        productId: id,
        title: title ? title : product?.title ? product.title  : "",
        categoryId: categoryId.id ? categoryId.id : product?.categoryId ? product.categoryId : "",
        model: model ? model : product?.model ? product.model : "",
        brand: brand ? brand : product?.brand ? product.brand : '',
        description: description ? description : product?.description ? product.description : '',
        price: price
          ? parseInt(price)
          : product?.price 
          ? product.price
        ...
        securityDeposit: securityDeposit
          ...
          ? product.securityDeposit
          : 0,
      }).then(()=>setEditPublication((prev)=>{return {...prev,saveButton:false}}));
      setEditShow({
        selectCategory: false,
        inputTitle: false,
        inputMarca: false,
    ...
    return (
      <Flex w='100%' mt={12}>
        ...
        <Flex>
          <Text fontSize="20px" fontWeight="semibold" ml="1%">Categoría:</Text>
          <Text ml="1%" mr="1%" color='blue.400'
              fontSize="20px"
              fontWeight="semibold">
          {editPublication.category ? editPublication.category : product?.category.name} 
          </Text>
          <button
            onClick={() =>
              setEditShow({
        ...
        <Flex>
          <Text fontSize="20px" fontWeight="semibold" ml="1%">Titulo:</Text>
        ...
        <Flex>
          <Text fontSize="20px" fontWeight="semibold" ml="1%">Marca:</Text>
          <Text ml="1%" mr="1%" color='blue.400'
              fontSize="20px"
              fontWeight="semibold">
        ...
        {editShow.inputMarca && (
         ...
         <Flex>
          <Text fontSize="20px" fontWeight="semibold" ml="1%">Descripción:</Text>
          <Text ml="1%" mr="1%" color='blue.400'
              fontSize="20px"
              fontWeight="semibold"
             w='80%'
            >
            {editPublication.description ? editPublication.description : product?.description}
          </Text>
          <Box h='20px' w='20px' mr='1%' mt='1%'>
          <button
            onClick={() =>
              setEditShow({
                ...editShow,
  ...
  <Flex justifyContent='center' w='100%'>
        <Box border='solid blue 2px' borderRadius='20px' h='fit-content' w='fit-content' mb='1%'>
        <Tabs pr='3%'  pl='3%'>
          <TabList >
          {
       ...
       return [positionPicture - 1,p]
        ...
        <Flex w="100%" justifyContent="center">
          ...
          borderRadius='30px'
           src={
            publicationPictures[p[0]][1] ? publicationPictures[p[0]][1] : p[1]
           }
            w="150px"
           h="150px"
          />)}
    ...
    >
      <Text>Agregar</Text>
          ...
          <TabPanels>
            <TabPanel>
              <Button
                fontSize="15px"
                fontWeight="semibold"
                size='sm' bgColor='blue.400' _hover={{bgColor:'blue.500'}}
                onClick={handleClick}
              >
                Cambiar
              </Button>
        ...
        <Box w='100%'>
        <Flex>
          <Text fontSize="20px" fontWeight="semibold" ml="1%">Modelo:</Text>
          <Text ml="1%" color='blue.400'
              fontSize="20px"
              fontWeight="semibold">
        ...
        <Flex>
          <Text fontSize="20px" fontWeight="semibold" ml="1%">Price:</Text>
          <Text ml="1%" color='blue.400'
              fontSize="20px"
              fontWeight="semibold">
        ...
        <Flex>
          <Text fontSize="20px" fontWeight="semibold" ml="1%">Seña:</Text>
          <Text ml="1%" color='blue.400'
              fontSize="20px"
              fontWeight="semibold">
            {editPublication.securityDeposit ? editPublication.securityDeposit :
             product?.securityDeposit}
          </Text>
          <Box justifyContent='center' mr='1%' ml='1%' mt='1%'>
          <button
